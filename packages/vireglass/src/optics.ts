 ...
 *
 * ИЗМЕРЕНО по тому, насколько цветная кромка на самом деле. В .reference/liquid-glass/frames/
 * crops самые яркие пиксели кромки дают разброс каналов (max − min) / max: 13% в cap158-left и
 * 20% в cap158-right — против 14.6% и 14.3% у их собственного фона. Кромка не цветнее сцены, в
 * которой стоит: «перелив» добавляет единицы процентов, а в одном из двух кадров — ничего.
 *
 * При 1.6 обычное стекло получало 0.945, и спектральный множитель доходил до 97% разброса:
 * красный в полтора раза вверх, синий в ноль. На цветной подложке это неоновая трубка.
 */
export const iridescence = (ior: number, filmNm: number) =>
  filmNm <= 0 ? 0 : clamp01(fresnelStrength(ior) * 0.042);
 ...
 * не видны вовсе.
 */
export const diffraction = (ior: number) => clamp01(dispersion(ior) * 0.05);
 ...
 */
export const colorPickup = (ior: number) => clamp(fresnelStrength(ior) * 0.9, 0, 0.3);
