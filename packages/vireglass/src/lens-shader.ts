...
half4 main(float2 xy) {
  ...
  // Показатель лепестка ИЗМЕРЕН по крупным планам реальных торцов (.reference/liquid-glass/
  // frames/crops/cap158-left и cap158-right): полная ширина яркой дуги на половине её пика —
  // 53° в первом кадре и 30° во втором. Ширина лепестка cos^n равна 2·acos(0.5^(1/n)), так что
  // это показатели 6 и 20. Было 3, чья дуга шириной 75° — шире обоих замеров, и ошибка в одну
  // сторону оба раза. Обе дуги вдобавок упираются в белое на вершине (38° и 9° клиппинга), так
  // что замеренные ширины — если и смещены, то в бóльшую сторону.
  //
  // 6 — осторожный конец: воспроизводит более широкий кадр точно. Один показатель не может
  // обслужить оба, и это не шум: ширина зеркального лепестка зависит от углового размера
  // источника не меньше, чем от поверхности, а это две разные сцены. Члена для размера
  // источника в модели нет.
  float key = u_specular * (pow(max(facing, 0.0), 6.0) + 0.45 * pow(max(-facing, 0.0), 6.0));
  float rimFacing = dot(n, L);
  float rimKey = u_specular * (pow(max(rimFacing, 0.0), 6.0) + 0.45 * pow(max(-rimFacing, 0.0), 6.0));
