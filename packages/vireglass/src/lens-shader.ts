...
half4 main(float2 xy) {
  ...
  if (u_iridescence > 0.001) {
    // Окно у самой кромки — по той же причине, по которой оно есть у дифракции ниже:
    // перелив живёт на встречной ДУГЕ, а дуга у силуэта. Без окна угол поверхности
    // перестаёт меняться на трети фаски, и плёнка красит остаток одним плоским цветом.
    float wIri = u_iridescence * smoothstep(0.25, 1.0, t);
    spectral *= mix(float3(1.0), vgInterference(clamp(N.z, 0.0, 1.0)), wIri);
  }
