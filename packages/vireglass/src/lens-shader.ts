...
// Сверено с эталоном по кадру и снижено втрое (docs/vireglass/benchmarks/2026-09-12-scatter-vs-reference.md):
// у Apple строка под капсулой остаётся строкой, у нас превращалась в ровное пятно.
const float VG_SCATTER_MAX = 0.06;
...
//
// Эти два числа долго противоречили строке выше: при 0.15/0.70 деталь с краской над текстом
// замазывала его на две трети, и от строки оставалось ровное пятно (сохранялось 10% контраста
// против эталонных 94%). То есть модель обещала рассеяние, а делала заливку — ровно ошибку
// из §12. Снижены до 0.06/0.12 по замеру, а читаемость осталась на своих настоящих опорах:
// размытии и потолке светлоты (needForInk ниже), которых правка не касается.
