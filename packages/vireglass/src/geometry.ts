...
const SIZE_GAIN_MIN = 0.8;
const SIZE_GAIN_MAX = 2.4;

/**
 * Насколько крупной читается деталь — для того, насколько толсто её стекло.
 *
 * Не `halfMinDp`: это половина узкой стороны, верная мера для предела фаски или радиуса
 * касания, но не для этого. У шита 390×420 и 390×780 узкая сторона одна и та же, а поверхности
 * разные. И не площадь: у линейки 900×8 её больше, чем у кнопки тулбара, а это самое тонкое
 * стекло в интерфейсе.
 *
 * Среднее геометрическое двух мер уважает оба факта: узкая сторона ограничивает, сколько стекла
 * вообще может быть, а среди фигур с одинаковой узкой стороной больше поверхности читается как
 * больше стекла. Выведено, не подобрано: константы не вводит.
 */
export const sizeDp = (g: VireGlassGeometry) =>
  Math.sqrt(halfMinDp(g) * Math.max(Math.sqrt(g.width * g.height) / 2, 1));

/**
 * Крупнее деталь — толще стекло: сильнее линза, глубже тень (reference.md §1, 219 @6:36 —
 * «по мере того как стекло перетекает в более крупные размеры, оно имитирует более толстый
 * материал с более глубокими тенями и более выраженным линзированием»). Фаска и толщина заданы
 * для детали опорного размера и растут как корень из него.
 *
 * Потолок достигается асимптотически, а не упором. Жёсткое отсечение делало все поверхности
 * выше него одним и тем же стеклом, а выше него живёт весь интерфейс: полуоткрытый шит, полный
 * шит, сайдбар iPad и панель Mac 900px сидели ровно на 2.4. Референс называет сайдбары КРУПНЫМ
 * концом диапазона, так что модель, переставшая отвечать до них, перестала ему подчиняться.
 *
 * Кривая ниже — тот же корень; стык на `SIZE_GAIN_MIN` непрерывен и по значению, и по наклону,
 * асимптота — `SIZE_GAIN_MAX`, как и было. Обе константы сохранили значения: изменилось то, что
 * насыщение стало плавным вместо отсечения.
 */
export const sizeGain = (g: VireGlassGeometry) => {
  const raw = Math.sqrt(sizeDp(g) / SIZE_REF_DP);
  if (raw <= SIZE_GAIN_MIN) return SIZE_GAIN_MIN;
  const span = SIZE_GAIN_MAX - SIZE_GAIN_MIN;
  return SIZE_GAIN_MIN + span * (1 - Math.exp(-(raw - SIZE_GAIN_MIN) / span));
};
