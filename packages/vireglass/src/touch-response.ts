...
/**
 * ПОДЪЁМ В СТЕКЛО (эталон §5, M 4:10–4:30). Орган, который в покое стеклом не был — ручка
 * свитча, ползунка, — под пальцем перестаёт быть матовым и становится линзой: сквозь него
 * проступает то, что под ним.
 *
 * Стекло приходит БЫСТРЕЕ, чем уходит матовость: доли перекрываются, иначе в середине
 * перехода сквозь ручку на миг видно подложку — дырка вместо органа управления.
 *
 * Отрыв от подложки сюда НЕ входит. `u_lift` шейдера — направление («деталь этого рода
 * вдавливается или поднимается»), а не величина: насколько высоко она уже поднялась, знает
 * нажатие, на которое `u_lift` и домножается. Отдавать отсюда третью долю значило бы
 * умножить нажатие на само себя — так и вышло на первом подключении веб-стенда.
 */
