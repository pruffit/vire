 ...
 * проступает то, что под ним. Деталь при этом отрывается от подложки, и тень отходит.
 *
 ...
 */
export function raiseIntoGlass(press: number): { glass: number; solid: number; lift: number } {
  const t = press < 0 ? 0 : press > 1 ? 1 : press;
  return { glass: Math.pow(t, 0.6), solid: 1 - Math.pow(t, 1.4), lift: t };
}
