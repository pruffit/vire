...
globalThis.vgInkProbe = ({ press }) => {
  ...
  const piece = {
    ...
    inkIdle: [1, 1, 1, 1],
    inkActive: [1, 1, 1, 1],
    // Палец уведён в угол детали: локальное искажение поля вокруг него до штриха не достаёт,
    // а расфокус от расстояния не зависит вовсе. Равномерную растяжку всей детали при нажатии
    // (vgTouchWarp делит координату на 1 + доля нажатия) это не убирает — она и даёт те 32%
    // смягчения, что остаются при выключенном расфокусе, и потому порог стоит выше них.
    // Радиус пятна тот же, что кладёт продукт: доля половины меньшей стороны.
