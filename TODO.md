...
- [~] **Пресейвы — Фаза B (смартлинки ↔ Vire + внешние платформы)** — Фаза A сделана.
  Ядро Фазы B готово (21.06.2026):
  ...
  - [x] Редактор смартлинка: дропдаун выбора релиза (любой статус, проверка владения
    в API POST/PATCH).
  - [ ] Кнопка пресейва инлайн в секции «Скоро выйдет» на странице артиста (нужен
    per-release presave-state — батч-запрос).
