...
- [ ] **Google Search Console** — завести сайт, верифицировать через `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` (build-arg, по аналогии с `S3_PUBLIC_ENDPOINT` в Dockerfile / deploy.yml), отправить sitemap `https://viremusic.ru/sitemap.xml`, запросить индексацию страниц. Без этого Google не индексирует сайт.
- [ ] **Яндекс.Вебмастер** — подтвердить метатег (v1.0.56), проверить FAQ и Article в разделе «Микроразметка».
