...
- [ ] **Explicit-бейдж (E) везде, где виден релиз/трек** — сейчас `ExplicitBadge` есть на
  странице трека и в трек-листах, но НЕТ на карточках релизов (пример: «Свежие релизы» на
  главной). Explicit — флаг по треку (`tracks.is_explicit`), у релиза прямого флага нет,
  поэтому: добавить агрегат `hasExplicit` (любой трек релиза explicit) в запросы карточек
  (`DiscoveryRelease`/`discovery.ts`, `feed.ts`, `editorial.ts`) и рендерить E в
  `ReleaseQuickLook` и прочих местах с обложкой релиза (главная, лента, профиль артиста,
  каталог `/releases`, смартлинки). Бейдж теперь с потолком размера (v1.0.71) — масштаб ок.
