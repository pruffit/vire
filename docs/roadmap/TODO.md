...
## Долг переработки плеера/рекомендаций (v1.8.0, осознанный — не блокеры)

- [ ] **Унификация трек-строк** — `track-list.tsx` (Row), `liked-track-row.tsx`,
  `playlist-track-row.tsx`, `purchased-track-row.tsx` дублируют обложку/оверлей/метаданные;
  свести к одному параметризуемому компоненту.
- [ ] **Сирота-sid волны** — `mintWaveSessionId` ротирует sid до фетча: при неудачном
  `startWave` продолжающая играть старая волна теряет серверный анти-повтор (клиентский
  `played` покрывает). Коммитить sid только при успехе.
- [ ] **`originalQueue` персистится без среза** (pre-existing B1) — длинная волна с
  шаффлом может писать >100 треков в localStorage.
- [ ] **Кэш `getTasteProfile`** — пересчитывается на каждый запрос волны (3 group-by);
  короткий memo/кэш при росте нагрузки.
- [ ] **Кнопка очереди мини-бара скрыта `<sm`** — на мобилке очередь только через
  фуллскрин (лишний тап). Продуктовый вопрос.
- [ ] **Live-очередь без потолка в памяти** — рантайм-`queue` не ограничен (персист
  режется окном 100); добавить кап и в live-состояние.
- [ ] **Restored показывает 0:00 до первого play** — duration/peaks не персистятся;
  UX-доводка при желании.
