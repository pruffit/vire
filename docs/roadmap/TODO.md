...
- [ ] **Тяжёлые og:image** — обложки уходят в OG как есть, без ресайза: смартлинк отдаёт
  11.9 МБ, релиз 2.1 МБ. Facebook режет фетч на 8 МБ, у Telegram/VK риск таймаута превью
  на медленном VPS. Нужен ресайз под 1200×630 (динамический `opengraph-image` или
  трансформация в S3-слое). Найдено аудитом 27.07.2026.
- [ ] **Мелочи разметки (аудит 27.07.2026)** — `mainEntityOfPage` у всех постов-`Article`
  указывает на одну страницу артиста (`lib/structured-data.ts`); публичные плейлисты без
  `MusicPlaylist`-разметки и вне sitemap (возможно, осознанно); canonical главной без
  завершающего слэша.
...
- [ ] **Джем: pending не различает свою команду и чужую** — `resolvePending` сравнивает только
  `paused`: если во время своего pending другой участник сменил состояние, кнопка показывает
  желаемое (не фактическое) до TTL 5с. Звук при этом синхронен (идёт от серверного playback).
  Чинится версионированием команд, если станет заметно (ревью 27.07.26).
