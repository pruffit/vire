...
- [x] **Проверка разметки** (27.07.2026) — аудит JSON-LD и OG/Twitter по живому проду
  ...
  (3) `og:title` не совпадал с `<title>` (шаблон `%s — Vire` к OG не применяется);
...
- [~] **Контент** — ✅ фолбэк meta-description релиза/трека уникализирован (чистый хелпер
  `apps/web/lib/meta-descriptions.ts`: варьирует текст по типу релиза/году/кол-ву треков,
  заданное `release.description` по-прежнему приоритетнее). Осталось: заполнить `bio` у
  артистов без описания — авторский контент, задача артиста/админки.
- [ ] **Смартлинк подтягивает данные неопубликованного релиза** (ревью 27.07.2026) —
  `getSmartLinkRelease` не фильтрует статус: если у лендинга пустые `title`/`coverUrl`, а
  `release_id` указывает на DRAFT/ARCHIVED, наружу (страница + OG-карточка) уходят название
  и обложка черновика. Существовало и до OG-карточек — те лишь повторили поведение страницы.
  **Чинить с осторожностью:** SCHEDULED-релиз тут легитимен (это путь пресейва), фильтровать
  нужно только DRAFT/ARCHIVED.
- [ ] **Скрытый артист с треками доступен по прямой ссылке** (ревью 27.07.2026) —
  `assertArtistVisible` (и зеркалящий его OG-роут) гейтит только по «нет опубликованных
  треков»; `artist_profiles.isActive=false` («скрыть с витрины» в `/admin/artists`) убирает
  артиста из каталога/поиска/sitemap, но прямой заход и OG-карточка всё равно отдают данные.
  Продуктовый вопрос: что именно значит «скрыть с витрины».
- [ ] **Sitemap без потолка** — ни одна секция (артисты, релизы, треки, смартлинки, теперь
  плейлисты) не ограничена по числу записей и не разбита через `generateSitemaps`. Упрётся
  в лимит протокола 50k URL при росте каталога.
...
- [ ] **Мобайл-фёрст UX/UI-база для всего сайта** — отдельный полный vire-loop.
