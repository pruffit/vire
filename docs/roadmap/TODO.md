...
- [x] **`/artists` упирается в 200 артистов** (18.08.2026) — страница дозапрашивает
  каталог постранично (`listAllArtists`, `apps/web/app/[locale]/(listener)/artists/
  (catalog)/page.tsx`, `limit=200` за вызов, до 25 страниц) вместо одного `limit:200`
  вызова — клиентский фильтр по жанру/поиску/сортировке (`ArtistCatalog`) по-прежнему
  видит весь каталог, счётчик `200+` убран (больше не нужен, список полный).
- [x] **`GET /api/v1/artists/[slug]` мимо гейта видимости** — закрыто коммитом `d30ed17a`
  (гейт `artistHasPublishedTrack`+`canViewEmptyArtist`, тот же, что у страницы). Пункт в
  TODO не был отмечен — найдено при ревизии беклога 18.08.2026, правка не потребовалась.
...
- [x] **Файлы в S3 не удаляются вместе с треком/релизом** (найдено и починено 16.08.2026,
  волна 6.1). Сделан фоновый уборщик: `storage_orphans` + часовой воркер `storage-cleanup`,
  grace-период сутки, ключи регистрируются ДО удаления строк. Проверено end-to-end на живом
  MinIO: до grace не трогает, после — удаляет мастер, манифест и сегменты, соседний префикс
  цел. Детали — `docs/features/storage-cleanup.md`.
  **Остаток:** файлы, осиротевшие ДО этой фичи, не убраны — записей о них нет, нужен разовый
  проход-ревизия (сверка ключей `tracks/*` в S3 с `tracks.id` в БД).
- [x] **Накат миграций с нуля падал** (найдено и закрыто 16.08.2026 при проверке перед
  деплоем). Мигратор drizzle применяет весь набор одной транзакцией, а Postgres запрещает
  использовать значение enum в транзакции, где оно добавлено: 0017 (`ADD VALUE 'PERSONAL'`)
  + 0051 (бэкфилл по этому значению) → `unsafe use of new value`. Прод не задет — там
  миграции приезжали порциями. Закрыто скриптом `db:migrate:fresh` (транзакция на миграцию,
  журнал в формате drizzle); проверено: 57 миграций на чистую базу, схема совпала с рабочей
  (44 таблицы), после него штатный мигратор считает базу актуальной.
  **Остаётся знать:** новые миграции, добавляющие значение enum и сразу его использующие,
  надо разносить по двум файлам — иначе тот же капкан повторится на проде.
- [ ] **Observability — внешний приёмник ошибок (Sentry/GlitchTip).** Отложено до машины побольше. sentry.io блокирует РФ (403), а self-hosted GlitchTip не влезает в
  2 ГБ RAM (celery-worker прожорлив, гарантированный OOM). Sentry-SDK из кода
...
- [ ] **Мажорные апгрейды зависимостей** (заведено 02.09.2026). Dependabot следит только
  за minor/patch: сгруппированные мажоры давали неревьюабельный PR на четыре несвязанные
  ...
  **Известно заранее: даже minor/patch по RN и Expo требуют трёх правок типов** (вскрылось
  ...
  | `bullmq` | 5 → 6 | очереди транскода и рассылок; проверять идемпотентность и `lockDuration` |
  | `ioredis` | 5 → 6 | и BullMQ, и presence (`lib/presence.ts`); деградация до `count:0` должна сохраниться |
  | `@sentry/react-native` | 7 → 8 | приёмник крашей свой (`sentry-envelope.ts`) — сверить формат envelope до апгрейда |
  | `react-native-gesture-handler` | 2 → 3 | жесты мобилки целиком |
  | `react-native-get-random-values` | 1 → 2 | источник энтропии для E2EE-ключей |
  | `motion` | 12 → 13 | анимации `packages/ui` |
  | `jsdom` | 29 → 30 | тестовое окружение web |
  | `typescript` | 5.9 → 7.x | нативный порт (tsgo), мажор через версию |
...
- [x] **Аудит `overflow-y-auto` без явной оси X** (найдено и закрыто 16.08.2026 при починке
  ...
  (бейдж на `absolute`, `ring-2`, тень) рисует лишнюю горизонтальную полосу. Было 26 мест —
  прошёл каждое: меню, поповеры, списки, панели чата/джема, очередь плеера, фуллскрин,
  быстрый просмотр получили `overflow-x-clip`; `dashboard/layout.tsx` — только `md:`,
  чтобы на узком экране широкие таблицы дашборда остались прокручиваемыми (как в
  `(listener)/layout.tsx`). Регресс закрыт инвариантом в `app/__tests__/layout-shell.test.ts`:
  у каждой вертикальной скролл-области в `app/**` и `components/**` должна быть задана ось X.
