...
- [ ] **Джем: optimistic pause/play у хоста** — toggle в `jam-room.tsx` читает `room.playback` из замыкания; быстрый двойной клик до SSE-подтверждения шлёт вторую `pause` вместо `resume`. Фикс: локальный optimistic-флаг, сбрасываемый SSE-состоянием (ревью 21.07.26).
- [ ] **Тест ретрай-таймера бутстрапа E2EE** — `useIdentity` ретраит ошибку раз в 15с (`e2ee-client.ts`); цикл таймера не покрыт юнитом с fake timers, существующие тесты проверяют только повтор через ре-монтаж (ревью 21.07.26).
...
- [ ] **Хвост клэмпа оверлеев (27.07.2026)** — `color-field.tsx`, `date-field.tsx`,
  `members-manager.tsx` держат свою разметку панели и получили только клэмп ширины
  (`max-w-[calc(100vw-2rem)]`), позиция остаётся `left-0` от триггера: у поля не
  у левого края правый край панели всё ещё может уехать. Закрывается переводом на
  примитив `Popover` (там уже `useViewportClampX`) при следующем проходе по формам.
