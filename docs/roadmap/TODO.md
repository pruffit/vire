...
- [ ] **Файлы в S3 не удаляются вместе с треком/релизом** (найдено 16.08.2026 при волне 6.1).
  `TrackService.deleteTrack` и `ReleaseService.deleteRelease` удаляют только строки БД —
  мастер в `vault` (`tracks/{id}/source.*`), HLS-сегменты и манифест в `stream`, обложки
  остаются в хранилище навсегда. Утечка растёт со всеми удалениями за всю историю проекта.
  Инструмент для починки уже есть: `IFileStorage.remove/stat` и единый `@vire/storage`
  (волна 6.1). Не сделано намеренно — удаление файлов необратимо, и порядок («сначала БД,
  потом файлы» против «пометить и убрать фоном») стоит выбрать явно: при падении между
  шагами либо остаётся мусор, либо теряется мастер живого трека. Предлагаемый путь —
  фоновая задача-уборщик по списку осиротевших ключей, а не удаление внутри запроса.
