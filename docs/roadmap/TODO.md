...
- [x] **Четыре тестовых файла флакали под нагрузкой** (найдено 03.09.2026 на прогоне мажоров, закрыто 04.09.2026).
  `lib/player/audio-engine.test.ts` и три `lib/jam/sources/*.test.ts` (soundcloud/youtube/vire).
  В изоляции 49/49 стабильно; в полном прогоне на загруженной машине дают от нуля до шести
  падений двух видов: `Test timed out in 5000ms` и `expected [FakeWidget, …] to have a length
  of 1 but got 2`. Второй симптом — накопление инстансов фейка между кейсами: либо модульный
  массив не сбрасывается, либо поздний колбэк загрузки скрипта из предыдущего теста долетает
  в следующий. Вредны не сами падения, а то, что они шумят в каждом прогоне гейтов и маскируют
  настоящие регрессы — на апгрейде `motion` они полчаса выглядели как поломка от мажора.
  **Причина — стоимость самих тестов, а не гонка в источниках:** `vi.resetModules()` в каждом
  кейсе переисполняет тяжёлый граф модулей (`@/store/player` тянет аудио-движок), и на Windows
  это не укладывается в дефолтные 5с — при лимите 2000мс падают 3 теста из 8, при 1200мс пять,
  в изоляции проходят все 49. `testTimeout` поднят до 20с в `apps/web/vitest.config.ts`.
  Массивы инстансов фейков заодно вынесены из статиков класса в переменную, захватываемую
  фабрикой на каждый тест, — **это укрепление, а не подтверждённый фикс:** воспроизвести
  каскад «инстансов 2 вместо 1» принудительным таймаутом не удалось, причинность не доказана.
  Проверка: два полных прогона под конкурентной сборкой — 2043/2043.
...
- [ ] **Мажорные апгрейды зависимостей** (заведено 02.09.2026). Dependabot следит только
  ...
  **Известно заранее: даже minor/patch по RN и Expo требуют трёх правок типов** (вскрылось
  ...
  |---|---|---|
  | `zod` | 3.25 → 4.x | несущая зависимость: `@vire/api-contracts` — единственный источник типов; ломается вывод типов во всём дереве. **Ждёт мержа мобильной ветки:** мобилка держит `zod` прямой зависимостью и импортирует его в плеере |
  | ~~`bullmq` 5 → 6~~ | ✅ 03.09.2026 | обошлось без правок кода: расписания уже на `upsertJobScheduler` (удалённые repeatable jobs не задели), `paused` в `getJobCounts` не запрашивается, `Queue#client` не используется. Опция `connection` жива — сводки release notes про обязательный `BackendFactory` врут, сверять по типам в `node_modules` |
  | ~~`ioredis` 5 → 6~~ | ✅ 03.09.2026 | мажор включил **RESP3 по умолчанию** — на своих шести клиентах закреплён `protocol: 2`, формат ответов остался v5. Границы `zrange` теперь только строками (`jam-state.ts`, `wave-session.ts`). Проверено живым прогоном против Redis: zrange/hgetall/multi/pub-sub/джоб/шедулер |
  ...
  | ~~`motion` 12 → 13~~ | ✅ 03.09.2026 | единственный breaking — убран `@emotion/is-prop-valid`; emotion/styled-components в репозитории нет |
  | ~~`jsdom` 29 → 30~~ | ✅ 03.09.2026 | только поднятый минимум Node (`^22.22.2 \|\| ^24.15.0 \|\| >=26`) |
  | `typescript` | 5.9 → 7.x | нативный порт (tsgo), мажор через версию |
  | `eslint` + `@eslint/js` | 9 → 10 | **заблокировано не нами** (проверено 03.09.2026). `eslint-config-next` тянет `eslint-plugin-react`, последняя версия 7.37.5 с peer по `^9.7`; плагин зовёт `context.getFilename()`, удалённый в ESLint 10, и падает на первом файле. Форсировать нечем. Возвращаться, когда `eslint-plugin-react` выпустит поддержку 10. **Вторая, уже наша половина:** `eslint-config-next` и `@vire/config/eslint/base` регистрируют плагин `@typescript-eslint` каждый из своей копии, что ESLint 10 запрещает — лечится фильтром `plugins` в `apps/web/eslint.config.mjs` при склейке конфигов |
