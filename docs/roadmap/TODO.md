...
- [ ] **Аудит `overflow-y-auto` без явной оси X** (найдено 16.08.2026 при починке полос
  прокрутки в сайдбаре слушателя). По спеке CSS `overflow-x: visible` рядом с
  `overflow-y: auto` вычисляется в `auto`, поэтому любой вылезающий за край элемент
  (бейдж на `absolute`, `ring-2`, длинная строка без `truncate`) даёт лишнюю горизонтальную
  полосу. Сейчас таких мест **26** (меню, поповеры, списки, панели чата и джема); починен
  только сайдбар, где баг проявился. Остальные надо проходить по одному — вслепую ставить
  `overflow-x-clip` нельзя: где-то горизонтальный скролл может быть намеренным.
  Список получается скриптом: `overflow-y-auto` без `overflow-x-*` в окне ±3 строки.
