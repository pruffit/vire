...
- [ ] **Кадр 0:00 текущего времени в resume-окне после F5** — `duration` уже
  персистится и не обнуляется (см. выше), но `currentTime` на первом кадре после
  регидрации ещё мелькает нулём до применения сохранённой позиции. Направление
  фикса: выставлять `hasAudio:true` в колбэке `MANIFEST_PARSED` **после** установки
  `currentTime` при `isResume`, не раньше. НЕ чинить через `isLoading` в fallback-ветке —
  это регрессирует buffer-stalls (уже проверено ранее).
- [ ] **Single-flight на конкурентные промахи TTL-кэша вкуса** — `getTasteProfile`
  (TTL-кэш, `packages/core/src/util/ttl-cache.ts`) при параллельных промахах на один
  ключ пересчитывает профиль несколько раз вместо дедупликации в один in-flight промис.
  Перф-мелочь, не корректность — опционально.
- [ ] **Собственные typecheck-скрипты для `packages/core` и `packages/db`** — сейчас
  типы этих пакетов проверяются только транзитивно через `pnpm --filter @vire/web
  typecheck`. Гигиена CI: пакетам нужен свой `tsc --noEmit`, чтобы ошибка не всплывала
  только на сборке web.
- [ ] **a11y-мелочи `TrackRow`** — статичный `aria-label="Воспроизвести"` не
  переключается на «Пауза» при активном воспроизведении; в `playlist-track-row.tsx`
  условие `durationSec && (...)` рендерит текстовый узел «0» при `durationSec === 0`
  вместо пустого рендера (falsy-число вместо булева условия).
...
- [ ] **Волновой скоринг `getWaveTracks` (`packages/db/src/queries/wave.ts`)** —
  8–10 коррелированных подзапросов вычисляются для КАЖДОГО видимого трека до
  `ORDER BY … LIMIT`. Самый нагруженный запрос платформы (волна догружается за
...
- [ ] **CSP `'unsafe-inline'` в `script-src` на проде** (`next.config.ts:46`) —
  сводит CSP как защиту от XSS почти к нулю. Требует перевода инлайн-скриптов Next
  на nonce; не косметика, отдельной задачей.
- [ ] **Guest-пресейв без double opt-in** (`presave/route.ts:54`) — можно ввести
  чужой email, на него уйдёт письмо при выходе релиза. Rate limit бьёт по самому
  email, а не по IP. Продуктовое решение.
- [ ] **`sessionId` присутствия не подписан** (`lib/presence.ts`) — счётчики
  «слушают сейчас» накручиваемы в пределах rate limit. Косметическая метрика;
  если оставляем — задокументировать как принятый риск.
- [ ] **XHR батч-загрузки не абортится при анмаунте** (`batch-track-upload.tsx`).
