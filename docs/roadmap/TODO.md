...
- [ ] **Soft-404: `notFound()` отдаёт 200** (найдено 28.07.2026 смоуком прод-артефакта).
  Несуществующий роут отдаёт честные 404, а `notFound()` внутри страницы (`/playlists/{uuid}`,
  `/artists/{slug}` и др.) рендерит `app/not-found.tsx` со статусом **200**. Для поисковика
  это soft-404: страница-заглушка может попасть в индекс. Не связано с коллаборативными
  плейлистами — воспроизводится на страницах, которых правка не касалась. Копать в сторону
  стриминга SSR (статус нельзя поменять после флаша шелла) и `proxy.ts`.
