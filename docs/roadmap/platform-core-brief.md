...
| 5 — Shared UI | ⬜ | | |
| 6 — SDUI Foundation | 🟡 в работе | 16.08.2026 | Волна 8 срез 1: контракт `sdui.ts`, чистая `composeHomeScreen` в `platform/sdui`, `GET /v1/screens/home`, реестр рендереров, главная за флагом `sdui.home` (выключен). Ветки сверены на прод-сборке — `docs/features/sdui-home.md`. Осталось: раскатка флага, проверка слоёв/TTFB на проде |
...
3. **Одна реализация, два входа.** Логика чтения — read-сервис в core; Server Component
   ...
   не гоняем (лишний хоп, потеря стриминга, 2 ГБ RAM).
4. **Core как отдельный сервис — не на 2 ГБ.** Сейчас набор пакетов
   в одном образе.
