...
2. **Продолжить слушать** (вошедшим) — недавно играные треки, горизонтальный
   рейл обложек, `RecentRail` + `CoverRail` → `getRecentlyPlayed`.
3. **Для тебя** (вошедшим) — треки артистов из лайков/подписок,
   `PlayableTrackList` (plain, 2 колонки) → `getPersonalTrackPicks`.
4. **Новое у подписок** (вошедшим) — свежие релизы артистов, на которых
   подписан слушатель, горизонтальный рейл `ReleaseQuickLook` → `getFeed`.
5. **Горячие треки** (всем) — топ по прослушиваниям за 30 дней, `HotTracks`
   (ranked-список) → `getPopularTracks`.
6. **Поток** (всем) — кнопка запуска **Волны** + mood-чипы, `FlowBlock`
   (см. [wave](wave.md)).
