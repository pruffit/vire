...
- **Автоприменение**: если у трека ещё нет ни одной строки в `track_genres` (артист
  ...
  confidence ≥ 0.05 (см. `decideAutoApplyGenres`, `AUTO_APPLY_THRESHOLD`). Запись
  идёт через `setTrackGenresIfEmpty` — проверка пустоты и вставка в одной транзакции,
  чтобы не затереть жанры, выбранные артистом за время инференса (TOCTOU-гонка). Порог
...
3. **Патчи и батчи**: кадры режутся на патчи по 128 фреймов (без перекрытия —
   ...
   гигантский тензор целиком (VPS 2 ГБ RAM).
