...
5. **Маппинг** (`apps/worker/src/lib/discogs-genre-map.ts` + `discogs-genre-labels.ts`)
   ...
   - **Агрегация — смешанная, не чистая сумма.** Зонтичные жанры (WORLD, POP,
     HIPHOP) собирают в разы больше Discogs-меток, чем точные (TRAP, SHOEGAZE,
     LOFI — по одной). При простом суммировании вероятностей диффузная масса
     двух десятков слабых меток зонтика перевешивала один уверенный точный
     лейбл, и модель предлагала зонтик там, где явно слышен точный поджанр.
     Score жанра = `max` его меток + `0.3 × (sum − max)` (константа
     `SUM_TAIL_WEIGHT` в коде) — остаток суммы всё ещё поднимает жанр, но не
     решает исход количеством меток. Confidence — доля score жанра в сумме
     score по всем замэпленным жанрам (не по всем 400 меток), топ-5 как раньше.
