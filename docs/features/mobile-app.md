...
- Друзья (список/заявки/поиск) — закрыто инкрементом 9. Просмотр профиля пользователя —
  ...
  там же). Чат 1:1 (нужен транспорт вместо browser-only `EventSource`), бейдж непросмотренных
  заявок, блокировка/разблокировка, просмотр содержимого плейлиста (свой или чужой, не
  существует вообще) — по-прежнему вне скоупа.
