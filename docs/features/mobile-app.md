...
- Друзья (список/заявки/поиск) — закрыто инкрементом 9 (см. выше). Просмотр чужого профиля,
  чат 1:1, бейдж непросмотренных заявок — сознательно вне скоупа, см. «Не проверено»
  инкремента 9.
- Пуши, биометрия, шеринг, виджеты.
