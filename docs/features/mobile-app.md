# Мобильное приложение (инкременты 1–6)

React Native + Expo клиент: auth → навигация → каталог → воспроизведение звука → SDUI-главная
+ нативная полировка → лайки и плейлисты. Инкремент 1 — дизайн в
...
- Shuffle/repeat-UI, оффлайн-скачивание — сами функции очереди в `@vire/core` уже есть на
  будущее, UI сознательно не добавлен. Лайки/добавление в плейлист — закрыто инкрементом 6
  (см. ниже).
