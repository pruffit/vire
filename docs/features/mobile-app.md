...
- **Android SDK/эмулятор — с инкремента 4 есть** (`VireMusic_Test`, API 36, `ANDROID_HOME=
  ...
  RNTP). Predictive back gesture, визуальный pull-to-refresh, end-to-end вход через Expo Go
  (не через прямую форму входа) — всё ещё не проверены живьём и остаются кандидатом для
  следующей сессии.
