...
тёмным его сменяет лёгкое затемнение, потому что тёмное стекло гасит контент само. Плотности
взяты у веба (`apps/web/rnd-src/scroll-edge.ts`), а не подобраны заново. На тёмной теме
...
Экран без списка о себе не сообщит, и сила залипла бы от предыдущего — поэтому на входе экрана
она сбрасывается к полной (`useScrollEdgeReset` в `components/screen.tsx`), а список тут же
уточняет её замером. Подключён ОСНОВНОЙ вертикальный список каждого экрана; горизонтальные
карусели и вложенные списки не трогаются — под мебель заезжает не их содержимое.
