...
- **Живой прогон на эмуляторе не пройден в этой сессии.** Первая попытка (отдельный агент)
  оборвалась серединой из-за лимита API-сессии; ручной подхват в главной сессии упёрся в
  окружение — после рестарта `expo start --dev-client` (Metro-процесс с прошлой сессии успел
  умереть сам, как уже бывало) под перезапущенным процессом пропали `EXPO_PUBLIC_API_BASE_URL`/
  `EXPO_PUBLIC_WEB_BASE_URL` (лог `.expo/dev/logs/start.log` показал пустой `env:load` вместо
  ожидаемых значений), а установленное приложение на повторный запуск не заявило Metro новый
  `metro:bundling:started` вовсе — экран оставался пустым белым дольше обычной задержки сплэша.
  Похоже на комбинацию известной хрупкости ручной настройки эмулятора (см. инкремент 9,
  «побочная находка окружения») с тем, что переменные окружения передавались через shell
  `export` перед фоновым запуском вместо файла `.env.local`, а не на баг кода — но это
  предположение, не подтверждённый факт. **Не заявлять «работает на эмуляторе», пока это не
  будет подтверждено следующим прогоном** (перезапуск через `.env.local` с явными
  `EXPO_PUBLIC_*`, либо `npx expo run:android` набело, если проблема повторится).
- Блокировка/разблокировка — сознательно без UI (см. «Что сделано» выше).
...
- Друзья (список/заявки/поиск) — закрыто инкрементом 9. Просмотр профиля пользователя —
  закрыто инкрементом 10 (живой прогон на эмуляторе не пройден в этой сессии из-за окружения,
  см. «Не проверено» там же — первый шаг следующей сессии). Чат 1:1 (нужен транспорт вместо
  browser-only `EventSource`), бейдж непросмотренных заявок, блокировка/разблокировка,
  просмотр содержимого плейлиста (свой или чужой, не существует вообще) — по-прежнему вне
  скоупа.
