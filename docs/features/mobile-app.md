...
Сила одна на приложение, поэтому её держит ВЛАДЕЛЕЦ — список сфокусированного экрана: на входе
он заявляет себя и пересчитывает силу из своего последнего замера, на выходе снимает заявку, и
без владельца (экран без списка) край возвращается к полному. Сбрасывать силу «на входе экрана»
оказалось нельзя: экран при потере фокуса не размонтируется, нативные `onLayout`/
