...
- **Режим похожести** (`trackId` передан — очередь исчерпывается во время волны):
  **закреплённый seed сессии (mood/genre) остаётся ЖЁСТКИМ фильтром, а не только
  ...
  артиста из последних 5 выданных), сессионный буст по mood (+0.35) и по genre —
  точный/семейный (ранжирует точный жанр выше соседей по семейству), шум (`random() * 0.15`).
...
- **`tasteGenreScore`/`tasteGenreScoreSeed`** (профиль вкуса): тот же `genreOverlapTerm` —
  ...
  `taste.topGenres`.
- **`sessionGenreBoost`**: поскольку сессионный genre теперь ещё и жёсткий фильтр (все
  кандидаты уже в семействе), буст лишь **ранжирует внутри** пула — точное совпадение с
  закреплённым genre даёт полный 0.35, только семейное — половину 0.175 (точный жанр
  всплывает выше соседей). `CASE` с приоритетом точного условия — не суммируются.
