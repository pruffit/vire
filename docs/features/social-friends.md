...
- **Известный редкий race:** `friendships` уникален по направленной паре `(requester_id,
  ...
  одновременно шлют заявку друг другу, в БД могут появиться два `PENDING`-ребра (A→B и B→A) —
  статус рассинхронизируется до ручного разрешения. Не порча данных, самовосстанавливается
  через accept/decline. Корректный фикс — канонизация пары (как `conversations.user_low/high`)
  или advisory-lock вокруг `request()`; отложено (смена уже отгруженной схемы `friendships`).
