...
- Dependabot-алерт GHSA-wrw7-89jp-8q8g (`glib` 0.18, unsound `VariantStrIter`) закрыт как
  «код не используется» 23.09.2026. Обновиться нельзя: `wry` закрепляет `webkit2gtk =2.0.2`,
  тот требует `glib ^0.18`; уязвимый итератор не вызывает ни один крейт дерева, а glib
  собирается только в Linux-сборке. Пересмотреть, когда `webkit2gtk` выйдет на `gtk` 0.19.
