...
ниже. Мини-плеер поверх других окон
(`docs/superpowers/specs/2026-08-17-desktop-mini-player-design.md`), см. «Мини-плеер»
...
- Отдельное окно мини-плеера (~300×110, always-on-top, без рамки), не требующее фокуса
  на главном окне — переключается пунктом трея «Мини-плеер», подробности в
  «Мини-плеер» ниже.
...
Отдельное окно поверх других (~300×110, `always_on_top`, без рамки, `skip_taskbar`),
не требующее фокуса на главном окне — переключается пунктом трея «Мини-плеер»
(`CheckMenuItem`, тот же паттерн, что автозапуск). Дизайн:
`docs/superpowers/specs/2026-08-17-desktop-mini-player-design.md`.

- **Синк без Tauri IPC.** Оба окна грузят один и тот же origin (`/desktop/mini-player` —
  top-level нелокализованный маршрут, как `/admin`); синк состояния идёт через
  `BroadcastChannel('vire-desktop-player')` — стандартный веб-API, работает между окнами
  одного WebView2-профиля без участия Rust. `apps/web/lib/player/desktop-sync.ts`:
  `initDesktopPlayerPublisher()` (главное окно, инициализируется в `useEffect`
  координатора плеера рядом с `initAudioEngine()`/`initMediaSession()`) — единственный
  источник правды, рассылает `{type:'state', state}` на изменение
  track/isPlaying/currentTime/duration и раз в секунду во время игры; отвечает на
  `{type:'requestState'}` и принимает команды `togglePlay`/`next`/`prev`, вызывая те же
  `controls.*`, что и `bridge.rs`. `useMiniPlayerState()` (окно мини-плеера) — подписка +
  локальная интерполяция позиции таймером между сообщениями (для плавного прогресс-бара)
  + отправка команд обратно тем же каналом.
- **Веб-страница без chrome.** `proxy.ts` — ветка `/desktop/mini-player` форвардит
  заголовок `x-desktop-chrome: none` (тот же приём, что `x-next-intl-locale`); корневой
  `app/layout.tsx` читает заголовок и на этом пути не монтирует
  Nav/PlayerWrapper/MobileTabBar/DeferredWidgets/чат-бутстрап/YandexMetrika — только
  `NextIntlClientProvider`+`MotionProvider`+`{children}`. Инвариант app-shell
  (`app/__tests__/layout-shell.test.ts`) не задет — диф чисто условный, `<body>`/
  `#main-content`/`<PlayerWrapper />` остаются в тексте файла.
- **Rust-окно** (`main.rs`) создаётся в `setup()` эагерно, скрытым (`.visible(false)`) —
  проще, чем ленивое создание по требованию, и не мешает `tauri-plugin-window-state`
  (не в денилисте — SIZE/POSITION/MAXIMIZED трекаются как у главного окна, `VISIBLE`
  глобально отключён у плагина ещё с среза 6). `CloseRequested` → `prevent_close` + `hide`
  + снятие чекбокса в трее — тот же паттерн, что у главного окна (срез 4).

**Проверено эмпирически (17.08.2026):**

- **`BroadcastChannel` реально долетает между окнами Tauri одного origin — в обе
  стороны**, подтверждено диагностическими каналами на `cargo tauri dev` (`localhost:3000`):
  сообщение, отправленное отдельным `BroadcastChannel` в контексте главного окна,
  принято отдельным слушателем в контексте окна мини-плеера, и наоборот.
- **Найдена и исправлена реальная гонка при старте.** Оба окна инициализируются
  параллельно при запуске приложения; `useMiniPlayerState()` слал `requestState` один
  раз при маунте, и если паблишер главного окна ещё не успел зарегистрировать
  `channel.onmessage` (типичная ситуация — окно мини-плеера объективно легче и
  маунтится раньше тяжёлого главного), ответ терялся навсегда: мини-плеер показывал
  «Ничего не играет» неограниченно долго — именно то, что предупреждение спеки просило
  проверить и не выпускать не проверенным. Исправлено ретраем `requestState` каждые
  500мс до первого ответа (`desktop-sync.ts`) — воспроизведено и подтверждено закрытым
  на локальном стенде: реальный трек в главном окне синхронизировался в мини-плеере
  (название/артист/прогресс) сразу после фикса.
- **Команды из мини-плеера реально достигают главного окна и меняют состояние реального
  плеера** — клик «Предыдущий трек» в окне мини-плеера сдвинул `queueIndex` персистед
  стора главного окна с 9 на 8 и сменил `track.id`/`title` на предыдущий трек очереди
  (проверено не по UI-тексту, а по значению `localStorage['vire-player']` до/после).
- **Пункт трея «Мини-плеер» реально показывает/прячет окно** (реальные клики через
  System.Windows.Automation по системному трею + собранный `cargo tauri build`,
  `--release`, боевой `viremusic.ru`): `EnumWindows`/`GetWindowRect` по PID —
  `visible=False` до клика, `visible=True` после, `visible=False` после повторного
  клика; процесс остаётся в `tasklist` (не убит) на всём протяжении.
- **Позиция/размер окна мини-плеера переживают перезапуск `.exe`** — `SetWindowPos` на
  конкретные координаты, полный выход через пункт трея «Выход» (не forced kill —
  `tauri-plugin-window-state` сохраняет на `RunEvent::Exit`, не на любое завершение
  процесса), повторный запуск — `GetWindowRect` вернул координаты в точности как перед
  выходом (одна и та же не-DPI-aware измерительная обвязка на входе и после, тот же
  принцип, что в срезе 5).
- **`cargo tauri build`** — оба бандла (`msi`/`nsis`) собираются без ошибок с этими
  изменениями (`--no-sign`, тот же флаг, что и раньше для локальной сборки без ключа).
- **Побочная находка вне скоупа этого среза, не исправлялась:** тихая проверка
  обновлений (срез 7, `check_for_updates`) перезапускается на **каждый** `on_page_load`
  главного окна, а не только на первый холодный старт — на реальном `cargo tauri dev` с
  несколькими `location.reload()` подряд (во время отладки этого среза) поверх боевого
  `latest.json` (там уже реальная версия — вышедшая `v1.51.1`) это дало несколько
  наложившихся друг на друга диалогов подтверждения обновления. Существующее поведение
  среза 7, не тронуто — задокументировано как наблюдение для будущего захода.
...
Второй независимый CI-джоб, `build-desktop-linux` (`ubuntu-latest`, `needs: gates`,
параллельно `build-desktop` и веб-деплою — Tauri v2 официально собирается под Linux
только на Linux-раннере) — `cargo tauri build`, тот же `bundle.targets: "all"` в
`tauri.conf.json`, что и у Windows-джоба, на Linux-раннере даёт сразу `.deb`/`.rpm`/
`.AppImage`. В GitHub Release идут все три; на `/download` и в стабильной MinIO-ссылке
(`downloads/desktop/linux/VireMusic-x86_64.AppImage`) — только AppImage: тот же
UX-принцип, что у Windows `.exe` — портативный формат без установки и без root.

Rust-фикс для кроссплатформенности: `main.rs` строка 1 (`windows_subsystem`) раньше
компилировалась безусловно на любой ОС по одному `not(debug_assertions)`, хотя атрибут
Windows-специфичен — под `cfg_attr` добавлено условие `target_os = "windows"`, иначе
релизная сборка на Linux не компилируется вовсе.

Пробелы, честно не закрытые в этом заходе:
1. **Сборка на Linux ни разу не запускалась и не проверялась вживую** (машина
   разработки — Windows, нет Linux-таргета и системных GTK-библиотек локально) —
   первый реальный сигнал будет на следующем теге `vX.Y.Z` в CI.
2. **Трей/глобальные хоткеи/автозапуск на Linux не подтверждены эмпирически** — плагины
   Tauri заявляют поддержку (X11 и Wayland, разные DE), но в отличие от Windows-среза,
   где каждый пункт проверен `cargo tauri dev` + реальными событиями ОС, для Linux такой
   проверки не было.
3. **Автообновление на Linux сознательно не подключено.** `plugins.updater.endpoints`
   в `tauri.conf.json` по-прежнему указывает только на Windows-манифест, ключа
   `linux-x86_64` там нет — `tauri-plugin-updater` на отсутствующей в манифесте
   платформе просто не находит обновление (`Ok(None)`), не паникует, но пункт трея
   «Проверить обновления» на Linux всегда будет отвечать «последняя версия». Причина —
   безопасное объединение двух параллельных CI-джобов (Windows и Linux) в один
   `latest.json` без гонки записи требует последовательной зависимости между ними,
   это отдельное решение с реальным весом, не принимается заодно с включением сборки.
...
- `apps/desktop/src-tauri/src/main.rs` — `shell_url()`, `show_and_focus()` (общая
  ...
  `check_for_updates()` и пункт трея «Проверить обновления» (срез 7), окно мини-плеера
  + чекбокс «Мини-плеер» в трее (мини-плеер).
...
- `apps/web/lib/player/desktop-sync.ts` — `initDesktopPlayerPublisher()`/
  `useMiniPlayerState()`, синк мини-плеера через `BroadcastChannel`, см. «Мини-плеер».
- `apps/web/app/desktop/mini-player/page.tsx`, `apps/web/app/desktop/layout.tsx` —
  страница окна мини-плеера, см. «Мини-плеер».
- `apps/web/proxy.ts`, `apps/web/app/layout.tsx` — ветка `/desktop/mini-player`
  (`x-desktop-chrome: none`), см. «Мини-плеер».
...
- macOS-сборка не подключена и не проверялась (машина разработки — Windows). Linux —
  CI-джоб `build-desktop-linux` есть («Linux» выше), но ни разу не запускался вживую.
- Dependabot-алерт GHSA-wrw7-89jp-8q8g (`glib` 0.18, unsound `VariantStrIter`) закрыт как
  «код не используется» 23.09.2026. Обновиться нельзя: `wry` (и наш 0.55, и последний 0.57)
  закрепляет `webkit2gtk =2.0.2`, тот требует `glib ^0.18`; уязвимый итератор не вызывает ни
  один крейт дерева, а glib собирается только в Linux-сборке. Пересмотреть, когда `webkit2gtk`
  уйдёт на glib 0.22 (через `gtk` 0.19).
