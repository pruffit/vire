...
## Заметки
- **Скачивание FLAC (Этап 2)** пока выключено в UI. Presigned-URL подписываются под
  внутренний `S3_ENDPOINT` (minio:9000) и из браузера недоступны. Перед стартом Этапа 2
  нужно подписывать под публичный хост (через Caddy) или вынести vault на внешний S3.
