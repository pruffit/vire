...
а он вырезан до апгрейда VPS (не влезает в 1 ГБ RAM — см. `TODO.md` «Observability»). Полумера
(request_id в логах, которые на 1 ГБ читаются только через `journalctl`, без аггрегации) — низкая
