...
**Goal:** Превратить слушательскую часть Vire в дашборд по модели Spotify — постоянный левый sticky-сайдбар с навигацией и медиатекой, мобильный нижний таб-бар, новая `/library`, ужатый `/profile`, переработанная главная, удалённая `/feed`, расширенный футер.
...
```tsx
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12 space-y-16">
      ...
      <h1 className="sr-only">Vire — независимая музыкальная площадка…</h1>
...
- [ ] **Step 2: Самокритика отдельным сабагентом (Sonnet)** — прожарить диф по Vire-чеклисту: мобилка (узкий вьюпорт, таб-бар + плеер стекаются, sticky-сайдбар не даёт второй скролл), дубли компонентов, утечки/ререндеры, app-shell (нет `min-h-screen`, `#main-content` единственный скроллер), краевые случаи (гость в сайдбаре, пустая медиатека, дашборд без футера и без сайдбара/таб-бара). Найденное — починить и перепрожарить.
