...
**Статус: закрыто.** A — `b30e42f`, B — `986da3d` + `d285056`, C — `76ad5d9`.
Добавился незапланированный срез D (`b581c88`): при ревью среза C выяснилось, что гейт
видит разрядку только в arbitrary-виде (`tracking-[…]`), а через шкальные классы
(`tracking-wide`/`wider`/`widest`) тот же паттерн жил ещё в 19 файлах — то есть зелёный
гейт ничего не доказывал. Регулярка расширена, места переведены на роли.
