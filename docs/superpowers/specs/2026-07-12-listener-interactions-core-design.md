...
- Порядок проверок в `POST tracks/[id]/moments`, `PUT tracks/[id]/moods`,
  `POST playlists/[id]/tracks`, `POST playlists/[id]/cover` нормализован под канон
  слоёв: zod-валидация тела теперь ДО проверки ресурса (400 приоритетнее 404/403).
  Асимметрия PUT vs POST из «Инвариантов» (владение→валидация у POST) не сохранена —
  оба идут «валидация→владение». Наблюдаемо только при одновременно невалидном теле
  и отсутствующем/чужом ресурсе.
- `PUT tracks/[id]/moods` при треке без релиза (недостижимо из-за FK) отдаёт 404
  вместо 403 — единый паттерн `authorizeTrackOwnership` (release не найден →
  `NotFoundError`), а не отдельная ветка под недостижимый случай.
- `getPublicLyrics` в порту `IListenerTrackRepository` типизирован `LyricLine[] | null`
  (в спеке выше — опечатка `string | null`).
