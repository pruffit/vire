...
### Рекомендация (на решение при ревью спеки)
Развести роли по семантике:
- **Джем** — эфемерная сессия, а не постоянный раздел → таб-бару не место. Оставить
  контекстной точкой входа: заметная плитка в `/library` + запуск из плеера/`AdaptiveMenu`.
- **Друзья** — постоянный социальный раздел с входящими заявками → **добавить 5-м табом**
  (иконка + бейдж заявок переезжает сюда с Медиатеки, где он не по адресу).

**Итог рекомендации:** таб-бар из **5 пунктов** — Главная / Поиск / Медиатека / Друзья /
Сообщения (`grid-cols-5`; на 360px ≈ 72px/пункт — вмещает иконку 20px + подпись `text-[11px]`).

**Альтернативы (выбрать на ревью):**
- **(Б)** 4 таба, 4-й пункт → «Ещё/Профиль»-хаб (Друзья + Джем + Профиль + Настройки).
  Масштабируемо под будущие разделы, но +1 переход до Друзей.
- **(В)** Оставить 4, только улучшить плитки — **Danya отклонил** (выбрал «пересмотреть»).

> Открытый под-вопрос для User Review Gate: **5 табов (реком.)** или **хаб «Ещё» (Б)**.
> От ответа зависит состав `mobile-tab-bar.tsx` и куда переезжает бейдж заявок.

`isListenerShellPath`/само-скрытие таб-бара и `useChatUnread`-бейдж сохраняются.
