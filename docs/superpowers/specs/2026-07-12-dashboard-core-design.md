...
Найдено самокритикой и исправлено (НЕ отклонение): profile — remove-флаг обязан
выигрывать у приложенного файла, как до рефакторинга (`removeAvatar=1` + битый файл
→ 200 и удаление, а не 400). Хендлер не валидирует файл при взведённом remove-флаге;
покрыто тестами route.test.ts.
