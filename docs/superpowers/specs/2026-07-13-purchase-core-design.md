...
1. Тело запроса (`returnUrl` из JSON) парсится в роуте до вызова сервиса — чистый
   парсинг без эффектов, порядок относительно проверок сервиса не наблюдаем.
2. Веб-адаптер (`lib/payment-gateway.ts`) маппит `confirmation?.confirmation_url ?? null`
   вместо небезопасного прямого чтения поля — гипотетический ответ ЮKassa без
   `confirmation` раньше уронил бы обработчик в 500, теперь отдаёт 200
   `{ confirmationUrl: null }`. Закреплено тестом адаптера.

## Итоги реализации

Коммиты: `91d9ff9` (core: `PurchaseService` + `IPurchaseRepository`/`IPaymentGateway`,
тесты сервиса), `a4eedaa` (правка порядка проверок — см. ниже), `d55edf5` (самокритика,
2 NIT).

Тесты: core 301 → 319 (+18), web 669 → 685 (+16: purchase 7, webhook 9). Все гейты
(typecheck×3, lint, check:routes, тесты core/db/web, build) зелёные.

Самокритика (свежий Sonnet-контекст, сверка 1:1 по диффу): 0 MAJOR/MINOR, 2 NIT —
исправлены коммитом `d55edf5`.

**Эпизод: порядок проверок `isConfigured`.** В J1-реализации `isConfigured()`
(проверка настройки ЮKassa) оказалась поднята на край до вызова сервиса — раньше
`hasPurchased`/`getPending`. Это наблюдаемое отклонение от 1:1: при ненастроенном
платёжном шлюзе владелец уже купленного трека получал бы 503 «Платёжный сервис не
настроен» вместо ожидаемого `{ ok: true, alreadyOwned: true }` — то есть проверка
владения перестала бы работать раньше проверки конфигурации. Самокритика поймала
расхождение при сверке диффа с дизайном. Исправлено переносом `isConfigured` в
метод порта `IPaymentGateway` внутри `PurchaseService`, так что порядок
`trackExists → hasPurchased → getPending → isConfigured → createPayment` живёт в
сервисе и закреплён order-тестами и в `purchase.test.ts` (core), и в
`purchase/route.test.ts` (web).
