...
Все пять закрыты 14.09 — чем именно, в разделе «Чем закрыто» ниже.
...
**1. Тень.** Расхождения по числам не оказалось: веб уже вёл плотность тени зондом, и оба конца
сходятся с эталоном (9.4% над ровным при 10.5% у ручки Apple, 19.3% над пёстрым при 19.9% над
текстом). Настоящая дыра была в устройстве — закон жил в веб-рендерере, и на Android его не
существовало. Переехал в ядро, включён на обеих платформах. Замер —
`benchmarks/2026-09-14-shadow-adaptation-vs-reference.md`.
...
**4. Шкала прозрачности.** Появилась: `applyGlassScale`, 0 — ultra clear, 1 — fully tinted, и в
точке `GLASS_SCALE_DEFAULT` материал в точности сегодняшний. Правятся СЛЕДСТВИЯ, а не причины, и
это вывод из замера: причины «плотность среды» в модели нет, `bodyDensity` выводится из поглощения
толщи и не превышает 0.03, поэтому шкала толщины двигала бы число, которого в кадре не видно —
первая версия так и не сдвинула ни окно, ни различимость. Гейт `check:optics` получил седьмое
обещание: шкала меряется ХОДОМ, а не точкой — к тонированному концу окно закрывается (89% → 10%),
деталь становится заметнее (24.9 → 84.0). **В продукт шкала не подключена**: ось в ядре и гейт по
диапазону есть, вызовов из веба и мобилки нет, экрана настроек с ползунком нет. Закрыта дыра в
модели и в гейте, а не пользовательская фича.
