...
- [~] **Мобилка** (React Native + Expo, Android) — инкременты 1–17: вход, воспроизведение
  ...
  переиспользует существующий канал `notify-external`) — все подтверждены живьём на
  эмуляторе `VireMusic_Test`. Открыто: реальное физическое устройство (только эмулятор),
  лок-скрин с включённым PIN, Android Auto, iOS (нет Mac), привязка реального Expo/EAS
  `projectId` + Firebase/FCM-кредов (пуши закодированы полностью, но без этого шага
  деградируют в no-op — см. инкремент 17). Детали и честная разбивка проверено/не
  проверено по каждому инкременту — `docs/features/mobile-app.md`.
...
(`docs/features/mobile-app.md`, читать хвост файла за актуальным инкрементом): привязка
реального Expo/EAS `projectId` + Firebase/FCM для инкремента 17 (пуши), затем то, что
ещё вне скоупа — реальное Android-устройство вместо эмулятора (когда появится),
мультидевайс E2EE (SAS-привязка), биометрия, iOS (нет Mac).
