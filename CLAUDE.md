...
только продуктовые развилки (что строим) и необратимое (деплой/удаление/push). Тяжёлую
работу (реализация/ресёрч/ревью/самокритика) делегируй сабагентам, не главной сессии.
...
- [x] `packages/core` — Result<T,E>, domain types, сервисы (Artist/Release/Track,
  Follow/ListenerTrack/TrackMoods/Playlist, ArtistPost/SmartLink, Wave/Search/Presave), репозитории
...
### Тесты (apps/web — 573, гонять `pnpm --filter @vire/web test`)
