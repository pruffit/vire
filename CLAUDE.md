...
(`docs/features/mobile-app.md`, читать хвост файла за актуальным инкрементом): инкремент
17 (пуши) закрыт и подтверждён живьём целиком, дальше то, что вне скоупа — реальное
Android-устройство вместо эмулятора (когда появится), мультидевайс E2EE (SAS-привязка),
биометрия, iOS (нет Mac).
