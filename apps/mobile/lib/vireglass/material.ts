...
/**
 * Стекло под крупным текстом (панель текста в плеере). Толще и плотнее продуктового: тонкая
 * среда не может нести белые строки поверх обложки и не должна пытаться — раньше её за это
 * догружали плоским скримом, который гасил и преломление, и кромку.
 *
 * `legibility` близка к единице: это и есть причина «на мне обязаны читаться надписи». Она
 * поднимает плотность тела ПО МЕСТУ — плотнее над светлым и пёстрым, прозрачнее над ровным
 * тёмным, — а не заливает деталь целиком.
 */
