...
export type GlassLensProps = {
  ...
  shaderSource: string;
  /** Размер ВИДИМОГО стекла в dp. Сама вьюха обязана быть больше него: кромка собирает свет
   *  СНАРУЖИ формы, а рассеяние размывает фон вокруг точки — без запаса шейдеру там нечего
   *  семплировать. По этому же габариту берётся прямоугольник зонда светлоты. */
  glassWidth: number;
  glassHeight: number;
