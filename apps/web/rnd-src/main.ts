...
/**
 * Число стеклянных поверхностей поверх подложки — переключатель для замера стоимости среды
 * (спека фона, «Бюджет»: цена умножается на число стёкол). Заданный явно, ОН ЗАМЕНЯЕТ обычные
 * детали вида на N одинаковых квадратов независимо от зоны — так один и тот же харнесс мерит
 * и живую среду, и любую статичную подложку при 0/1/3/6 стёклах.
 */
...
/** Бенчмарк: рендерер обязан рисовать каждый rAF, а не останавливаться, когда кадр «устаканился»
 *  (обычный режим экономит кадры — иначе мерить время кадра было бы нечем: рендер просто не
 *  вызывался бы). */
...
/** `?advectSpeed=` — контрольный прогон гейта структуры на другой скорости течения (замер:
 *  накопленная численная диффузия зависит от пройденного пути, не только от схемы). Без
 *  параметра — дефолт `MEDIUM_DEFAULTS.advectSpeed`. */
...
if (benchMode) {
  // Замер стоимости среды (спека фона, «Бюджет»): время между кадрами САМОГО цикла rAF —
  // ровно то, что стоит один render() при текущем числе стёкол, — и, если браузер даёт
  // GPU-таймер-запросы, время GPU оттуда же (`renderer.getLastGpuMs()`).
  ...
  // Гейт на мигание (`scripts/medium-flicker-gate.mjs`): покадровая светлота небольшого окна
  // канваса, 60 кадров. Средний по кадрам снимок баг «переворот буфера каждый шаг» не ловит —
  // ловит только знак приращения между соседними кадрами, отсюда и покадровое чтение здесь.
  ...
  // Гейт равновесия плотности: сырая сумма пар+конденсат+следы в обход композита (спека фона,
  // «Конденсация: воды две») — вода (пар и конденсат вместе) сохраняется, у следов есть свой
  // распад, требование только к воде. `null`, пока `render()` с включённой средой ни разу не
  // вызывался.
  (
  ...
  // Сырая сетка ПАРА напрямую из рендерера (тройки r/g/b — три вида — на ячейку, не сумма):
  // гейт структуры (`scripts/medium-structure-gate.mjs`) читает контраст между видами. Гейт на
  // едущий узор её не использует — сверяет формулу шума напрямую, без рендера (снос
  // сохраняющейся массы иначе забивает сигнал самой функции шума).
  (
    window as unknown as {
...
/**
 * N одинаковых стеклянных квадратов в ряд — харнесс замера (`?glass=0|1|3|6`), общий для
 * ЛЮБОЙ зоны: заменяет обычные детали вида, чтобы один и тот же набор поверхностей можно было
 * положить и на среду, и на статичную подложку и сравнить цену (спека фона, «Бюджет»).
 */
