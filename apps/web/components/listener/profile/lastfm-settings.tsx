...
export function LastfmSettings({ initial }: { initial: string | null }) {
  ...
  async function save() {
    ...
    try {
      ...
      toast.error('Не удалось сохранить ник Last.fm');
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="rounded-xl border border-border bg-card/60 px-4 py-3.5 space-y-3">
      <div className="flex items-center justify-between gap-4">
        ...
        >
          {saved ? 'Привязано' : 'Не привязано'}
        </span>
      </div>
      <p className="text-xs text-muted-foreground">
        Берём только список любимых треков для предложки на вечеринке. Ничего не публикуем и не проигрываем оттуда.
      </p>
      <div className="flex items-center gap-2">
        ...
        >
          {pending ? '…' : willUnlink ? 'Отвязать' : 'Сохранить'}
        </button>
      </div>
      {!valid && trimmed !== '' && (
        <p className="text-xs text-red-400">Только латиница, цифры, «. _ -», от 2 до 64 символов</p>
      )}
    </div>
  );
}
