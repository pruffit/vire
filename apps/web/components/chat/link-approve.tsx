...
export function LinkApprove({ viewerId }: { viewerId: string }) {
  const t = useTranslations('chat.linkApprove');
  ...
  }, [identity.priv, identity.needsLink, approve, t]);
        ...
        <>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { void postLink('abort', { linkId: approve.linkId }); setApprove(null); setTyped(''); }}
              className="min-h-11 rounded-lg border border-border px-4 text-sm">{t('cancel')}</button>
          </div>
        </>
      ) : (
