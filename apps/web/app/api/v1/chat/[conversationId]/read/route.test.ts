import { describe, it, expect, vi, beforeEach } from 'vitest';
import { NotFoundError, ForbiddenError } from '@vire/core';

const { markRead } = vi.hoisted(() => ({ markRead: vi.fn() }));

vi.mock('@/auth', () => ({ auth: vi.fn() }));
vi.mock('@/lib/chat', () => ({ chatService: () => ({ markRead }) }));

import { auth } from '@/auth';
import { POST } from './route';

const mockedAuth = vi.mocked(auth);

const SELF_ID = '22222222-2222-2222-2222-222222222222';
const CONV_ID = '33333333-3333-3333-3333-333333333333';

const ctx = (id: string) => ({ params: Promise.resolve({ conversationId: id }) });
const req = () => new Request(`http://localhost/api/v1/chat/${CONV_ID}/read`, { method: 'POST' });

beforeEach(() => vi.clearAllMocks());

describe('POST /api/v1/chat/[conversationId]/read', () => {
  it('401 when not authenticated', async () => {
    mockedAuth.mockResolvedValue(null as never);
    const res = await POST(req(), ctx(CONV_ID));
    expect(res.status).toBe(401);
    expect(markRead).not.toHaveBeenCalled();
  });

  it('400 on invalid conversationId', async () => {
    mockedAuth.mockResolvedValue({ user: { id: SELF_ID } } as never);
    const res = await POST(req(), ctx('nope'));
    expect(res.status).toBe(400);
  });

  it('404 when not a participant', async () => {
    mockedAuth.mockResolvedValue({ user: { id: SELF_ID } } as never);
    markRead.mockResolvedValue({ ok: false, error: new NotFoundError('Conversation', CONV_ID) });
    const res = await POST(req(), ctx(CONV_ID));
    expect(res.status).toBe(404);
  });

  it('403 when forbidden', async () => {
    mockedAuth.mockResolvedValue({ user: { id: SELF_ID } } as never);
    markRead.mockResolvedValue({ ok: false, error: new ForbiddenError('Not a participant') });
    const res = await POST(req(), ctx(CONV_ID));
    expect(res.status).toBe(403);
  });

  it('marks read', async () => {
    mockedAuth.mockResolvedValue({ user: { id: SELF_ID } } as never);
    markRead.mockResolvedValue({ ok: true, value: undefined });
    const res = await POST(req(), ctx(CONV_ID));
    expect(res.status).toBe(200);
    expect(markRead).toHaveBeenCalledWith(SELF_ID, CONV_ID);
  });
});
