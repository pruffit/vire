import type { Metadata } from 'next';
import { Link } from '@/i18n/navigation';
import { LegalDoc } from '@/components/content-kit';

export const metadata: Metadata = {
  title: 'Политика конфиденциальности',
  description: 'Как VireMusic обрабатывает персональные данные пользователей',
};

export default function PrivacyPage() {
  return (
    <LegalDoc
      title="Политика конфиденциальности"
      revision="Редакция от 5 июля 2026 г."
      sections={[
        {
          title: 'Какие данные мы собираем',
          body: (
            <>
              <p>При регистрации через Яндекс (OAuth) мы получаем:</p>
              <ul>
                <li>email-адрес;</li>
                <li>отображаемое имя;</li>
                <li>URL аватара (хранится ссылка, изображение не копируется).</li>
              </ul>
              <p>При регистрации по email (пароль или ссылка magic-link) мы получаем только email-адрес.</p>
              <p>Если вы загружаете собственный аватар, изображение сохраняется в нашем файловом хранилище (S3).</p>
              <p>При пресейве анонсированного релиза без входа в аккаунт мы получаем только email-адрес, указанный для уведомления о выходе.</p>
              <p>В процессе использования Сервиса мы фиксируем:</p>
              <ul>
                <li>лайки треков (track_id + user_id);</li>
                <li>лайки плейлистов (playlist_id + user_id);</li>
                <li>подписки на артистов (artist_id + user_id);</li>
                <li>созданные плейлисты и их содержимое;</li>
                <li>пресейвы релизов: user_id у зарегистрированных пользователей либо email у гостей;</li>
                <li>события прослушивания (play events): track_id, анонимный session_id, дата/время, длительность и источник прослушивания. User_id привязывается только для зарегистрированных пользователей;</li>
                <li>любимые моменты на треке (таймкод + анонимный session_id, user_id — только для зарегистрированных);</li>
                <li>факт присутствия «слушают сейчас» — анонимный session_id в оперативном хранилище (Redis), не сохраняется дольше 45 секунд бездействия;</li>
                <li>mood-теги и жанры, проставленные треку (только артистами-владельцами трека).</li>
              </ul>
            </>
          ),
        },
        {
          title: 'Для чего используются данные',
          body: (
            <ul>
              <li>Аутентификация и идентификация пользователя в рамках Сервиса.</li>
              <li>Формирование персональной ленты подписок.</li>
              <li>Профиль вкуса для алгоритма подбора треков («Волна») — агрегируется из истории прослушиваний и лайков (mood-теги и жанры треков) для персональных рекомендаций.</li>
              <li>Уведомление о выходе релиза при пресейве и автоматическое добавление релиза в лайки в день выхода.</li>
              <li>Аналитика для артистов: количество прослушиваний, уникальные слушатели, динамика по дням.</li>
              <li>Обеспечение безопасности и предотвращение злоупотреблений (в том числе ограничение частоты запросов по IP-адресу).</li>
            </ul>
          ),
        },
        {
          title: 'Передача данных третьим лицам',
          body: (
            <>
              <p>Мы не продаём и не передаём персональные данные третьим лицам в рекламных или коммерческих целях.</p>
              <p>Данные могут передаваться только:</p>
              <ul>
                <li>провайдерам инфраструктуры (хостинг, хранилище файлов, email-рассылки) — в объёме, необходимом для работы Сервиса;</li>
                <li>по требованию уполномоченных государственных органов в установленных законом случаях.</li>
              </ul>
            </>
          ),
        },
        {
          title: 'Cookie, хранилище браузера и аналитика',
          body: (
            <>
              <p>Мы используем необходимые cookie — сессионный токен Auth.js для авторизации пользователя.</p>
              <p>Для сбора статистики о посещаемости на платформе подключена <strong className="text-foreground">Яндекс.Метрика</strong> (АО «Яндекс», Россия). Метрика собирает:</p>
              <ul>
                <li>данные о просмотренных страницах, источниках переходов, времени на сайте;</li>
                <li>клик-карты и карты скролла (clickmap);</li>
                <li>записи сессий пользователей — Вебвизор (движения мыши, клики, прокрутка). Вебвизор не фиксирует ввод в поля форм (пароли, email).</li>
              </ul>
              <p>Данные обрабатываются Яндексом в соответствии с <a href="https://yandex.ru/legal/confidential/" target="_blank" rel="noopener noreferrer">политикой конфиденциальности Яндекса</a>. Мы не передаём Яндексу персональные данные (email, имя) — только поведенческие данные браузерной сессии.</p>
              <p>localStorage используется для хранения состояния плеера (очередь воспроизведения, громкость, позиция трека) и предпочтений интерфейса (например, факт закрытия баннера). Эти данные хранятся только в вашем браузере и на сервер не передаются.</p>
            </>
          ),
        },
        {
          title: 'Хранение и удаление данных',
          body: (
            <>
              <p>Данные хранятся на серверах в России (Timeweb Cloud VPS). Файлы — обложки, аудиофайлы, аватары — хранятся в S3-совместимом хранилище там же.</p>
              <p>Для удаления аккаунта и связанных с ним персональных данных воспользуйся <Link href="/feedback">формой обратной связи</Link>. Мы обработаем запрос в течение 14 дней.</p>
              <p>После удаления аккаунта анонимизированные play events (без user_id) могут сохраняться в агрегированной статистике.</p>
            </>
          ),
        },
        {
          title: 'Безопасность',
          body: (
            <p>Мы применяем стандартные технические меры защиты: HTTPS, httpOnly-cookie для сессии, хранение файлов в закрытом S3-бакете (доступ только по подписанным URL).</p>
          ),
        },
        {
          title: 'Изменения политики',
          body: (
            <p>О существенных изменениях в политике конфиденциальности мы уведомим пользователей по email или через уведомление в Сервисе.</p>
          ),
        },
        {
          title: 'Контакт',
          body: (
            <p>По вопросам обработки персональных данных: <Link href="/feedback">форма обратной связи</Link></p>
          ),
        },
      ]}
    />
  );
}
